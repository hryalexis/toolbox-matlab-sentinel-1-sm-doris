...
%   - roi (optional): structure defining the region of interest to read in the
%       data file (see Additional information below).
%   - verbose (optional): logical flag to display progress information
%       (default: false).
%
% Outputs:
...
%   The 'roi' structure defining the region of interest (ROI) should contain the
%   following fields:
...
%   When 'roi' is not provided, the entire image is read from the data file.
%
% Author: Louis-Philippe Rousseau (Université Laval)
% Created: May 2014 (original name was 'getTIFFdata')
...
for ni = 1:nargin-1
    ...
    if isstruct( varargin{ni} )
        ...
        roi = varargin{ni};
    elseif islogical( varargin{ni} )% || isnumeric( varargin{ni} )
        % logical: verbose flag
        verbose = varargin{ni};
        %validateattributes( verbose, {'logical', 'numeric'}, {'scalar', ...
            %'nonnegative'}, '', 'verbose', ni+1 );
    else
